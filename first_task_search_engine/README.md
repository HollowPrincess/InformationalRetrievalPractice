...
Теперь вместо запроса **and** выполняется запрос **or**.
При построении эмбеддингов запроса удаляются ключевые слова и слова запроса, которые находятся после запросов с **not**.
...
## 2.2. Исправление опечаток.

Для испраления опечаток используется **deeppavlov** модель **spelling_correction.brillmoore_wikitypos_en**.